...
export class Splittwocolumns extends Component {
  ...
  renderEdit() {
    const tag_list = this.props.tagsList.map( ( tag ) => ( { label: tag.name, value: tag.id } ) );
    tag_list.unshift( { label: '--Select Tag--', value: 0 } );

    const issuepage_list = this.props.issuepageList.map( ( issue ) => ( { label: issue.title.raw, value: issue.id } ) );
    ...
    if ( focus_issue_image ) {
      const [ x, y ] = focus_issue_image.replace( /\%/g, '' ).split( ' ' );
    ...
    if ( focus_tag_image ) {
      const [ x, y ] = focus_tag_image.replace( /\%/g, '' ).split( ' ' );
