...
export class Takeactionboxout extends Component {
  constructor( props ) {
    ...
    const tagTokens = props.tagsList.filter( ( tag ) => props.tag_ids.includes( tag.id ) ).map( ( tag ) => tag.name );
  ...
  renderEdit() {
    ...
    const actpage_list = this.props.actPageList.map( ( actPage ) => ( { label: actPage.title.raw, value: actPage.id } ) );
    const { __ } = wp.i18n;
