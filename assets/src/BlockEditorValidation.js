...
const isValid = ( element ) => {
  ...
  if (
    'campaign' === $( '#post_type' ).val() &&
    'required' === $( element ).data( 'validation' )
  ) {
...
const PrePublishCheckList = () => {
  ...
  elements.forEach( ( element ) => {
    element.classList.remove( 'cmb2_required_field_error' );
  } );
  invalidElements.forEach( ( element ) => {
    element.classList.toggle( 'cmb2_required_field_error' );
  } );
  ...
  if ( postIsValid ) {
    ...
    checkListMsg.push(
      __(
        'Please check "Analytics & Tracking" section for required fields.',
