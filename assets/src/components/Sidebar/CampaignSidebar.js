...
import {
  fromThemeOptions,
  getFieldFromTheme,
} from '../fromThemeOptions/fromThemeOptions';
...
const Select = compose( fromThemeOptions, withPostMeta )( SelectControl );

const Radio = compose( fromThemeOptions, withPostMeta )( RadioControl );
...
export class CampaignSidebar extends Component {
  ...
  async handleThemeSwitch( metaKey, value, meta ) {
    ...
    const invalidatedFields = this.state.theme.fields
      .filter( ( field ) => {
        const resolvedField = getFieldFromTheme(
          this.state.theme,
          field.id,
          meta,
        );
        ...
        return ! resolvedField.options.some(
          ( option ) => option.value === currentValue,
        );
      } )
      .map( ( field ) =>
        getFieldFromTheme( this.state.theme, field.id, meta ),
      );

    return invalidatedFields.reduce(
      ( result, field ) => {
        return {
          ...result,
    ...
    wp.data.subscribe( () => {
      ...
      if (
        ...
        const parentPage = parentId
          ? wp.data
            .select( 'core' )
            .getEntityRecord( 'postType', 'campaign', parentId )
          : null;
        ...
        >
          { this.state.parent ? (
            <div className="components-panel__body is-opened">
              ...
              <p>
                { __(
                  'Style and analytics settings from the parent page will be used.',
            ...
            <>
              ...
              <PanelBody
                ...
                initialOpen={ true }
              >
                <Radio metaKey="campaign_nav_type" theme={ this.state.theme } />
                <ColorPalette
                ...
                <Radio
                  ...
                  help={ __(
                    'Change the campaign logo color (if not default)',
                    'planet4-blocks-backend',
                  ) }
                  theme={ this.state.theme }
                />
